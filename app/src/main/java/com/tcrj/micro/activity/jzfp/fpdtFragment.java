...
public class fpdtFragment extends BaseFragment implements BaseQuickAdapter.OnItemClickListener {

    public RecyclerView mRecyclerView;
    PtrFrameLayout mPtrFrameLayout;

    private MyOkHttp mMyOkhttp;
    private fpdtAdapter detailAdapter;
    ...
    protected void setView() {
        ...
        id = ACache.get(getContext()).getAsString("jpushId");
        ...
        mRecyclerView.setAdapter(detailAdapter = new fpdtAdapter(beanList, mContext));
...
//                      Toast.makeText(mContext, response.getMessage(), Toast.LENGTH_SHORT).show();
                        ...
                        if("9999".equals(errorCode)){
                            ...
                            fpjlListInfo response = new Gson().fromJson(dataBean.getData(), fpjlListInfo.class);
    ...
    //上拉加载更多数据
    private void loadMoreData(fpjlListInfo response,boolean isError) {
        ...
        if (response == null) {
            ...
            if(pageNum > response.getTotalPages()){//没有更多数据
                ...
                List<fpjlListInfo.ContentBean> content = response.getContent();
    ...
    public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
        fpjlListInfo.ContentBean item = (fpjlListInfo.ContentBean) adapter.getItem(position);

        Bundle bundle = new Bundle();
        bundle.putSerializable("fpdtinfo",item);
        toClass(getContext(),FpdtInfoActivity.class,bundle);
